...
module streamline_operator_5(
    ...
    input   wire    [8*10-1:0]    i_MAC_WEIGHT_PACKED,
    ...
    output  wire    [32*10-1:0]     o_out
);


wire signed [7:0]  i_MAC_WEIGHT[0:9];
`UNPACK_ARRAY(8, 10, i_MAC_WEIGHT_PACKED, i_MAC_WEIGHT, unpk_wrom)


//wire signed [31:0]  o_MAC_OUT[0:127];
//`PACK_ARRAY(32, 128, o_MAC_OUT, o_out, pk_o_data)
wire signed [31:0]  o_MAC_OUT[0:9];
`PACK_ARRAY(32, 10, o_MAC_OUT, o_out, pk_o_data)
    

wire signed [31:0] mac_output[0:9];

wire signed [31:0] acc_out[0:9];

    genvar i;
    generate
        for(i=0; i<10; i=i+1) begin : mac_gen
            streamline_acc_1_norelu    ACC_MODE_0  (
            .i_CLK                          (i_CLK),
            .i_ACC_RST                      (i_ACC_RST),
            .i_ACC_WR                       (i_ACC_WR),
            .i_ACC_RD                       (i_ACC_RD),
